...
type Server struct {
	...
	store  db.Store
	api    brewviewv1connect.BrewViewServiceHandler
}
...
func (s Server) NewGrpcHttpServer(addr string) (*http.Server, error) {
	...
	mux.Handle(brewviewv1connect.NewBrewViewServiceHandler(
		s.api,
		interceptors,
		compress1KB,
	))
	mux.Handle(grpchealth.NewHandler(
		grpchealth.NewStaticChecker(brewviewv1connect.BrewViewServiceName),
		compress1KB,
	))
	mux.Handle(grpcreflect.NewHandlerV1(
		grpcreflect.NewStaticReflector(brewviewv1connect.BrewViewServiceName),
		compress1KB,
	))
