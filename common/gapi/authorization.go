...
func (api *Api) authorizeUser(header http.Header) (*auth.Payload, *redisclient.AuthSession, error) {
	...
	log.Debug().Interface("asd", payload).Msg("hola")

	authSession, err := api.redisClient.AuthSessionFind(context.Background(), payload.Username)
	if err != nil {
		_, err := api.accountsUsecase.UserLogin(context.Background(), &rmsv1.UserLoginRequest{UserEmailOrCode: payload.Username})
		if err != nil {
...
func (api *Api) GetAccessableActionsForGroup(header http.Header, group string) (*rmsv1.ListDataOptions, error) {
	...
	log.Debug().Interface("authoritiesauthorities", authorities).Bool("is", authorities[strcase.ToCamel(create)]).Msg("gello")
