...
seed: 
	curl -X POST ${SEEDER_URL}/accounts/permissions && curl -X POST ${SEEDER_URL}/accounts/roles && curl -X POST ${SEEDER_URL}/accounts/users && curl -X POST ${SEEDER_URL}/accounts/navigations && curl -X POST ${SEEDER_URL}/storage && curl -X POST ${SEEDER_URL}/public
...
mock:
	mockgen -package mockdb -destination common/db/mock/store.go github.com/meloneg/brew_view_api/common/db/gen Store
sqlc :
...
dtag:
	docker tag brewview_api exploremelon/brewview_api:${v}

dpush:
	docker push  exploremelon/brewview_api:${v}
...
gen:
	rm -rf lib/pb/*  && protoc -I=proto/mln_api_protos --dart_out=grpc:lib/pb proto/mln_api_protos/brewview/v1/*.proto proto/mln_api_protos/brewview/v1/*/*.proto  proto/mln_api_protos/google/protobuf/*.proto
